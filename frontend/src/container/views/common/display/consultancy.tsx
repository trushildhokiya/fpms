...
import { FileDown, Info, Pencil, Table, Trash2Icon } from 'lucide-react'
...
const ConsultancyDisplay = (props: Props) => {
    ...
    const convertDates = (consultancies: Consultancy[]) => {
        ...
        return consultancies.map(consultancy => ({
            ...
            transactionDetails: convertTransactionDates(consultancy.transactionDetails)
        }));
    };
    ...
    const manageTransactionData = (data: Consultancy[]) => {
    ...
    const idBodyTemplate = (rowData: Consultancy) => {
        return <Badge className='bg-teal-400 bg-opacity-45 hover:bg-teal-300 text-teal-800'>{rowData._id}</Badge>;
    };


    const coInvestigatorBodyTemplate = (rowData: Consultancy) => {
        return rowData.coInvestigators.join(", ")
    }

    const departmentInvolvedBodyTemplate = (rowData: Consultancy) => {
    ...
    const facultyInvolvedBodyTemplate = (rowData: Consultancy) => rowData.facultiesInvolved.join(", ")

    const budgetBodyTemplate = (rowData: Consultancy) => <NumericFormat value={rowData.budgetAmount} thousandSeparator=',' readOnly disabled prefix='₹' />

    const sanctionedBodyTemplate = (rowData: Consultancy) => <NumericFormat value={rowData.sanctionedAmount} readOnly disabled thousandSeparator=',' prefix='₹' />

    const grantBodyTemplate = (rowData: Consultancy) => <NumericFormat value={rowData.totalGrantReceived} readOnly disabled thousandSeparator=',' prefix='₹' />

    const descriptionBodyTemplate = (rowData: Consultancy) => {
        return <ScrollPanel className='w-full h-36 text-sm leading-6'>{rowData.description}</ScrollPanel>
    }

    const startDateBodyTemplate = (rowData: Consultancy) => rowData.startDate.toLocaleDateString()

    const endDateBodyTemplate = (rowData: Consultancy) => rowData.endDate.toLocaleDateString()


    const sanctionedProofBodyTemplate = (rowData: Consultancy) => {
        return (
    ...
    const completionBodyTemplate = (rowData: Consultancy) => {
        return (
    ...
    const supportingBodyTemplate = (rowData: Consultancy) => {
        return (
    ...
    const transactionBodyTemplate = (rowData: Consultancy) => {
        return (
    ...
    const consultancyIDBodyTemplate = (rowData: TransactionData) => {
        return <Badge className='bg-orange-400 bg-opacity-45 text-orange-800 hover:bg-orange-300'>{rowData.consultancy_id}</Badge>
    }
    ...
    const allowExpansion = (rowData: Consultancy) => {
        return rowData.transactionDetails.length > 0;
    }

    const rowExpansionTemplate = (data: Consultancy) => {
        return (
            <div className="p-3">
    ...
    const actionBodyTemplate = (rowData: Consultancy) => {
        return (
            <>
                <Link to={`/common/edit/consultancy/${rowData._id}`}>
        ...
        <div>
            ...
            <div className="container font-Poppins my-10">
                ...
                <div className="my-10">
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className='tracking-wide font-bold text-gray-700 text-3xl py-2'>My Consultancy</CardTitle>
                            <CardDescription>Consultancy projects details of the faculty is shown below</CardDescription>
                        </CardHeader>
                        ...
                        <CardContent className='font-Poppins'>
                            ...
                            <DataTable
                                ...
                                exportFilename='my-consultancys' ref={dt} header={header} footer={footerTemplate} value={data} scrollable removableSort sortMode='multiple'
                    ...
                    <Card className='my-10'>
                        ...
                        <CardContent className='font-Poppins'>
                            ...
                            >
                                <Column style={{ minWidth: '250px' }} body={consultancyIDBodyTemplate} field="consultancy_id" header="Consultancy ID" filter filterPlaceholder='search by id' sortable></Column>
...
export default ConsultancyDisplay