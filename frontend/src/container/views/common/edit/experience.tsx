...
const experienceSchema = z.object({
    ...
    experienceProof: z.union([pdfFileSchema, z.any().optional()])
...
const ExperienceForm = (props: Props) => {
    ...
    useEffect(() => {
        axios.get('/common/experience')
            .then((res) => {
                setFormData(res.data.experience)
                form.reset({
                    experienceDetails: res.data.experience.map((exp: exp) => ({
                        ...exp,
                        fromDate: new Date(exp.fromDate),
                        toDate: new Date(exp.toDate),
                        experienceProof: new File([], '') // Handle file prepopulation as needed
                    }))
                })
            })
            .catch((err) => {
