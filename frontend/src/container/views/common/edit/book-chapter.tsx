...
const formSchema = z.object({
  ...
  intendedAudience: z.string().min(2, {
  ...
  proof: z.union([pdfFileSchema, z.any().optional()]),
});
...
const BookChapterForm: React.FC = (props: Props) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`/common/book-chapter/${id}`)
      .then((res) => {

        const data: BookChapter = res.data
        form.reset({
          ...data,
          authors: data.authors.join(', '),
          authorsAffiliation: data.authorsAffiliation.join(', '),
          facultiesInvolved: data.facultiesInvolved.join(', '),
          dateOfPublication: new Date(data.dateOfPublication),
        })
      })
      .catch((err) => {
        console.error("Error fetching book chapter data:", err);
      });
  }, []);
